...
//     { keyword: "amherst manor -", official: "1525 Amherst Manor Blvd, 111, Amherst, NY 14221" },
//     { keyword: "triads", official: "Triad Apartments, 1400 Millersport Hwy, Williamsville, NY 14221" },
//     { keyword: "niagara falls blvd", official: "Niagara Falls Blvd, Buffalo, NY 14214" },
//     { keyword: "niagara blvd", official: "Niagara Falls Blvd, Buffalo, NY 14214" },
//     { keyword: "niagara", official: "Niagara Falls State Park , 332 Prospect St, Niagara Falls, NY 14303" },
//     { keyword: "niagara falls", official: "Niagara Falls State Park , 332 Prospect St, Niagara Falls, NY 14303" },
//     { keyword: "niagara outlet", official: "Fashion Outlets of Niagara Falls USA , 1900 Military Rd, Niagara Falls, NY 14304" },
...
//     { keyword: "affinity ln", official: "102 Affinity Ln, Cheektowaga, NY 14215" },
//     { keyword: "gruner rd", official: "54 Gruner Rd, Cheektowaga, NY 14227" },
...
function InputForm({ inputs, setInputs, onCalculate, isLoading }) {
    ...
    // Attach Google Places Autocomplete after script is loaded
    useEffect(() => {
        if (!window.google || !window.google.maps || !window.google.maps.places) return;

        const options = {
            componentRestrictions: { country: "us" }, // Restrict to US (can remove if not needed)
            fields: ["formatted_address", "geometry", "name"],
        };

        if (startInputRef.current) {
            const autocompleteStart = new window.google.maps.places.Autocomplete(startInputRef.current, options);
            autocompleteStart.addListener("place_changed", () => {
                const place = autocompleteStart.getPlace();
                if (place?.formatted_address) {
                    setInputs((prev) => ({ ...prev, startPointInput: place.formatted_address }));
                }
            });
        }

        if (endInputRef.current) {
            const autocompleteEnd = new window.google.maps.places.Autocomplete(endInputRef.current, options);
            autocompleteEnd.addListener("place_changed", () => {
                const place = autocompleteEnd.getPlace();
                if (place?.formatted_address) {
                    setInputs((prev) => ({ ...prev, destinationInput: place.formatted_address }));
                }
            });
        }
    }, [setInputs]);
    ...
    return (
        <div className="space-y-6" style={{ position: "relative" }}>
            {/* Ride text */}
            <div>
                <label htmlFor="rideText" className="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="relative">
                    <label htmlFor="startPointInput" className="block text-sm font-medium text-gray-700 mb-1">
                        Starting Point
                    </label>
                    <input
                        type="text"
            ...
            {/* Manual Distance */}
            <div>
                <label htmlFor="manualDistance" className="block text-sm font-medium text-gray-700 mb-1">
                    Manual Distance (miles)
                </label>
                <input
                    type="number"
                    id="manualDistance"
                    value={inputs.manualDistance || ""}
                    onChange={handleChange}
                ...
                <p className="text-xs text-gray-500 mt-1">
                    This distance is from START to END. Use this if automatic distance lookup fails (replaces Google Maps distance).
                </p>
            </div>
            ...
            <div className="flex items-center justify-center pt-2">
                <input
                    id="roundTripCheck"
                    type="checkbox"
                    checked={inputs.roundTripCheck}
                    onChange={handleChange}
                    className="h-4 w-4 text-blue-600 border-gray-300 rounded"
                />
                <label htmlFor="roundTripCheck" className="ml-2 block text-sm font-medium text-gray-900">
                    Display as Round Trip?
                </label>
            </div>
            ...
            <div>
                <button
                    onClick={onCalculate}
                    disabled={isLoading}
                    className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 transition disabled:bg-blue-400"
                >
                    {isLoading ? "Calculating..." : "Calculate Ride Price"}
                </button>
            </div>
