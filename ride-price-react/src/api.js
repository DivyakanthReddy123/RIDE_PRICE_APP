...
// export const HOME_ADDRESS = "Triad Apartments, 1400 Millersport Hwy, Williamsville, NY 14221";
...
// " - 'amherst manor -': 1525 Amherst Manor Blvd, 111, Amherst, NY 14221",
// " - 'triads': Triad Apartments, 1400 Millersport Hwy, Williamsville, NY 14221",
...
//
// Case 1: Full ride request (natural language)
//
export async function getRideDetailsFromAI(text, apiKey) {
    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${apiKey}`;
...
//
// Case 2: Expand short manual names ("triads", "amherst") into official addresses
//
export async function expandAddressesWithAI(start, end, apiKey) {
    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${apiKey}`;

    const systemPrompt = `
You are an assistant that expands short or shorthand location names into official addresses in Buffalo, NY.

Input: JSON { "start": "...", "end": "..." }
- Expand each using the same mapping rules as before.
- If value is already a full address, leave it unchanged.
- If missing, set it to null.

Example Input:
{ "start": "triads", "end": "amherst" }

Example Output:
{ 
  "start": "Triad Apartments, 1400 Millersport Hwy, Williamsville, NY 14221", 
  "end": "1525 Amherst Manor Blvd, 111, Amherst, NY 14221" 
}
    `;

    const payload = {
        contents: [{ parts: [{ text: JSON.stringify({ start, end }) }] }],
        systemInstruction: { parts: [{ text: systemPrompt }] },
        generationConfig: { responseMimeType: "application/json" }
    };
